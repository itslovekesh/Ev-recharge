...
const ChargerProfile = () => {
    ...
    const { cToken, profileData, setProfileData, getProfileData } = useContext(ChargerContext)
    ...
    const updateProfile = async () => {
        ...
        try {
            ...
            const { data } = await axios.post(backendUrl + '/api/charger/update-profile', updateData, { headers: { cToken } })
        ...
        <div>
            <div className='flex flex-col gap-4 m-5'>
                ...
                <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
                    ...
                    <div>
                        <p className='flex items-center gap-1 text-sm font-medium text-[#262626] mt-3'>Location :</p>
                        <p className='text-sm text-gray-600 max-w-[700px] mt-1'>
                            {
                                isEdit
                                    ? <textarea onChange={(e) => setProfileData(prev => ({ ...prev, location: e.target.value }))} type='text' className='w-full outline-primary p-2' rows={8} value={profileData.location} />
                    ...
                    <p className='text-gray-600 font-medium mt-4'>
                        Price : <span className='text-gray-800'>{currency} {isEdit ? <input type='number' onChange={(e) => setProfileData(prev => ({ ...prev, fees: e.target.value }))} value={profileData.fees} /> : profileData.fees}</span>per hr
                    </p>
